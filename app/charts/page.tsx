...
type ChartModel = { ticker: string; price: number; score: number; direction: "Bullish" | "Bearish" | "Neutral"; candles: Candle[]; contract: string; dataMode: "live" | "demo"; warning?: string };
type MarketPayload = { ticker: string; price: number; candles: Candle[]; dataMode: "live" | "demo"; provider: string; warning?: string };
...
function buildChart(raw: string, timeframe: Timeframe, market?: MarketPayload | null): ChartModel {
  const ticker = normalize(raw);
  const candles = market?.candles?.length ? market.candles : demoCandles(ticker, timeframe);
  const price = market?.price ?? candles[candles.length - 1].close;
  const first = candles[0].close;
  const score = Math.round(50 + Math.min(45, Math.abs((price - first) / first) * 900 + (seed(ticker) % 24)));
  ...
  return { ticker, price, score, direction, candles, contract: `${ticker} 2026-07-17 ${strike} ${direction === "Bearish" ? "PUT" : "CALL"}`, dataMode: market?.dataMode ?? "demo", warning: market?.warning };
}
...
function MiniChart({ model, timeframe }: { model: ChartModel; timeframe: Timeframe }) {
  ...
  return <div style={{border:"1px solid rgba(103,232,249,.14)",background:"rgba(2,6,23,.45)",borderRadius:12,padding:10}}><svg viewBox={`0 0 ${width} ${height}`} style={{width:"100%",height:260,display:"block"}}>
    ...
    <text x={padding} y={18} fill="#94a3b8" fontSize="11">{timeframe} {model.dataMode === "live" ? "Alpaca" : "demo"} candles</text>
...
function ChartCard({ symbol, timeframe, onSymbol }: { symbol: string; timeframe: Timeframe; onSymbol: (value: string) => void }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/market/${normalized}?timeframe=${timeframe}`, { cache: "no-store" })
      .then((response) => response.json())
      .then((data: MarketPayload) => { if (!cancelled) setMarket(data); })
      .catch(() => { if (!cancelled) setMarket(null); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [normalized, timeframe]);

  const model = useMemo(() => buildChart(symbol, timeframe, market), [symbol, timeframe, market]);
  const tone = model.direction === "Bearish" ? "red" : model.direction === "Bullish" ? "green" : "blue";
  return <section className="panel"><div className="search" style={{marginBottom:14}}><input value={symbol} onChange={(event)=>onSymbol(event.target.value.toUpperCase())} onKeyDown={(event)=>{if(event.key==='Enter')onSymbol(normalize(symbol))}} aria-label={`${model.ticker} chart symbol`} /><button onClick={()=>onSymbol(normalize(symbol))}>{loading ? "Loading" : "Load"}</button></div>{model.warning ? <p className="subtle" style={{marginBottom:12,color:"#fde68a"}}>{model.warning}</p> : null}<div className="row" style={{alignItems:"end",justifyContent:"space-between",marginBottom:12}}><div><div className="kicker">Chart</div><h2 style={{fontSize:32}}>{model.ticker}</h2></div><div style={{textAlign:"right"}}><div className="price" style={{fontSize:26}}>{money(model.price)}</div><span className={`value ${tone}`}>{model.direction}</span></div></div><MiniChart model={model} timeframe={timeframe} /><div className="stats" style={{marginTop:12}}><div className="stat"><div className="label">Setup Score</div><div className={model.score >= 75 ? "value green" : "value amber"}>{model.score}/100</div></div><div className="stat"><div className="label">Best Contract</div><div className="value blue">{model.contract}</div></div><div className="stat"><div className="label">Mode</div><div className={model.dataMode === "live" ? "value green" : "value amber"}>{model.dataMode === "live" ? "Live Alpaca" : "Demo data"}</div></div></div></section>;
}

export default function ChartsPage() {
  const [timeframe, setTimeframe] = useState<Timeframe>("15m");
  const [symbols, setSymbols] = useState(starters);
  return <main className="page"><div className="shell"><header className="header"><div className="brand"><div className="logo">OP</div><div><h1>Four-Stock Charts</h1><p className="subtle">Compare four symbols at once with synchronized Alpaca candles when configured.</p><nav className="nav"><Link href="/">Analyzer</Link><Link href="/watchlist">Watchlist Scanner</Link><Link href="/market-radar">Market Radar</Link><Link href="/journal">Journal</Link><Link href="/pricing">Pricing</Link></nav></div></div><div className="badges"><span className="badge">Alpaca-ready</span><span className="badge">No live trading</span></div></header><section className="panel" style={{marginTop:16}}><div className="row" style={{alignItems:"center",justifyContent:"space-between"}}><div><div className="kicker">Timeframe</div><p className="subtle">Switch all four charts together.</p></div><div className="actions">{timeframes.map((item)=><button key={item} className={timeframe===item?"primary":""} onClick={()=>setTimeframe(item)}>{item}</button>)}</div></div></section><section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(100%,520px),1fr))",gap:16,marginTop:16}}>{symbols.map((symbol,index)=><ChartCard key={index} symbol={symbol} timeframe={timeframe} onSymbol={(value)=>setSymbols((current)=>current.map((item,itemIndex)=>itemIndex===index?normalize(value):item))} />)}</section><footer className="footer">Educational tool only. Not financial advice. Options involve significant risk. Charts are decision-support only and do not place trades.</footer></div></main>;
}
